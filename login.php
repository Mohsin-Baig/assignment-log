...
if(isset($_POST['login'])){
 ...
 $log_query = "SELECT * FROM `user` WHERE email = '$log_email'" ;
 $log_conn = mysqli_query($connection, $log_query);

 

 if(mysqli_num_rows($log_conn) > 0 ){
   $row = mysqli_fetch_assoc($log_conn);
   // $reg = mysqli_query($connection,$row);
   session_start();
   $_SESSION['useremail'] = $row['email'];
   $db_pass = $row['password'];
   $pass_check = password_verify($log_pass,$db_pass);
   // $conn = mysqli_query($connection,$pass_check);
   header('location:http://localhost:80/log/index.php');
 }else{
